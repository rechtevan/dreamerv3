...
**Issue Tracking**: Report issues to the upstream repository at [danijar/dreamerv3](https://github.com/danijar/dreamerv3/issues).
...
DreamerV3 is a scalable and general reinforcement learning algorithm that learns a world model from experiences and uses it to train an actor-critic policy from imagined trajectories. This is a JAX-based reimplementation that masters diverse control tasks with fixed hyperparameters.
...
- `--configs debug` - Fast debugging mode (reduced network/batch sizes, shorter intervals)
...
**Rationale**: Coverage focuses on core functionality used in production (agent, world model, training loops, environments). Visualization, interactive tools, and performance benchmarks are tested manually.
...
- `run/` (5 files): Training scripts (train.py, train_eval.py, parallel.py, eval_only.py)
...
- **Slow value (slowval)**: Target network for stable training (exponential moving average)

**Training Flow**:
1. `embodied.Driver` collects environment experience with parallel workers
2. Experience stored in `embodied.replay.Replay` buffer with configurable selectors
3. `agent.train()` samples batches, computes losses (world model + actor-critic)
4. World model trained on reconstruction, dynamics, reward prediction
5. Actor-critic trained on imagined rollouts using the world model (λ-returns)
6. Metrics logged, checkpoints saved periodically

### Config System

All hyperparameters defined in `dreamerv3/configs.yaml`:
- `defaults`: Base configuration used by all tasks
- **Task-specific configs**: `atari`, `crafter`, `dmc_vision`, `minecraft`, `dmlab`, etc.
- **Model size configs**: `size1m`, `size12m`, `size25m`, `size50m`, `size100m`, `size200m`, `size400m`
- **Special configs**: `debug` (fast iteration), `multicpu` (CPU parallelism)

Configs are merged in order specified via `--configs` flag using regex-based overrides for nested parameters (e.g., `.*\.rssm: {deter: 512, hidden: 256}`).
...
2. Register in `make_env()` constructor dict in `dreamerv3/main.py`:
   ```python
   ctor = {
...
3. Add environment-specific config block in `dreamerv3/configs.yaml`:
   ```yaml
   yourenv:
...
4. Add any dependencies to `requirements.txt` and `Dockerfile`

5. Add tests in `embodied/tests/test_yourenv.py`
...
3. **Version compatibility**: Pin versions in `requirements.txt`
4. **JAX compatibility**: Ensure compatibility with JAX ecosystem
...
✅ **MIT LICENSE file** - Covers all code with Danijar Hafner copyright
✅ **Git history** - Provides detailed attribution (use corporate email if applicable)
✅ **Preserves upstream attribution** - Maintains original license and copyright
❌ **NO copyright headers** - Not required, matches existing codebase

This is the same approach used by thousands of MIT-licensed projects and simplifies multi-contributor collaboration.
...
2. Creating entirely new subsystems (new algorithm implementations)
3. Upstream files already have headers (preserve and do not remove)
...
2. **Commit messages**: Clear, concise, explanatory
...
**Repository**: `rechtevan/dreamerv3` (fork of `danijar/dreamerv3`)
**License**: MIT License
**Language**: Python 3.11+
**Framework**: JAX, Optax, Ninjax
**Test Framework**: pytest
**Linting**: Ruff, MyPy
**Coverage Target**: 80-90%+ for core production code
